...
export async function getAdminAiDashboardData(): Promise<{
  ...
  try {
    const [overviewRow, inviteRows, ledgerRow] = await Promise.all([
      db
        .select({
          ...
          activeCodes:
            sql<number>`coalesce(sum(case when ${aiInviteCodes.status} = 'active' then 1 else 0 end), 0)::int`,
          disabledCodes:
            sql<number>`coalesce(sum(case when ${aiInviteCodes.status} = 'disabled' then 1 else 0 end), 0)::int`,
    ...
    console.error("Failed to load AI admin dashboard data:", error);
    return {
      overview: {
        totalCodes: 0,
        activeCodes: 0,
