...
const providers = {
  deepseek: {
    createUrl: (q: string) =>
      `https://chat.deepseek.com/?${new URLSearchParams({
        q,
      })}`,
    icon: (
      <svg
  ...
  kimi: {
    createUrl: (q: string) =>
      `https://kimi.com/?${new URLSearchParams({
        q,
      })}`,
    icon: (
      <svg
  ...
  gemini: {
    createUrl: (prompt: string) =>
      `https://gemini.google.com/app?${new URLSearchParams({
        prompt,
      })}`,
    icon: (
      <svg
      ...
      >
        <path
          d="M10 2.5l1.9 4.1 4.6.6-3.3 3.2.8 4.5L10 12.9 6 14.9l.8-4.5-3.3-3.2 4.6-.6L10 2.5z"
          opacity="0.2"
        />
        <path
          d="M10 5.2l1.1 2.5 2.8.4-2 2 .5 2.8-2.4-1.3-2.4 1.3.5-2.8-2-2 2.8-.4L10 5.2z"
          fill="currentColor"
        />
      </svg>
    ),
...
export const OpenInContent = ({ className, ...props }: OpenInContentProps) => (
  <DropdownMenuContent
    align="start"
    className={cn(
      "w-[240px] max-w-[calc(100vw-2rem)] border border-border/60 bg-background/90 backdrop-blur-md text-foreground shadow-2xl",
      className
    )}
    {...props}
  />
...
export type OpenInDeepSeekProps = ComponentProps<typeof DropdownMenuItem>;

export const OpenInDeepSeek = (props: OpenInDeepSeekProps) => {
  const { query } = useOpenInContext();
  return (
    <DropdownMenuItem asChild {...props}>
      <a
        className="flex items-center gap-2"
        href={providers.deepseek.createUrl(query)}
        rel="noopener"
        target="_blank"
      >
        <span className="shrink-0">{providers.deepseek.icon}</span>
        <span className="flex-1">{providers.deepseek.title}</span>
        <ExternalLinkIcon className="size-4 shrink-0" />
      </a>
    </DropdownMenuItem>
  );
...
export type OpenInKimiProps = ComponentProps<typeof DropdownMenuItem>;

export const OpenInKimi = (props: OpenInKimiProps) => {
  const { query } = useOpenInContext();
  return (
    <DropdownMenuItem asChild {...props}>
      <a
        className="flex items-center gap-2"
        href={providers.kimi.createUrl(query)}
        rel="noopener"
        target="_blank"
      >
        <span className="shrink-0">{providers.kimi.icon}</span>
        <span className="flex-1">{providers.kimi.title}</span>
        <ExternalLinkIcon className="size-4 shrink-0" />
      </a>
    </DropdownMenuItem>
  );
...
export type OpenInGeminiProps = ComponentProps<typeof DropdownMenuItem>;
  ...
  return (
    <DropdownMenuItem asChild {...props}>
      <a
        ...
        target="_blank"
      >
        <span className="shrink-0">{providers.gemini.icon}</span>
