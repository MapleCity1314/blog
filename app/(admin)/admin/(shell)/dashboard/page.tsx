...
export default async function AdminDashboardPage() {
  const [stats, postSummaries] = await Promise.all([
    getAdminDashboardStats(),
    getPostSummaries(),
  ]);

  const topPosts = postSummaries.slice(0, 3);
  const topPostViews = await Promise.all(
    topPosts.map(async (post) => ({
      title: post.metadata.title,
      views: (await getPostViews(post.slug)).toString(),
      engagement: "—",
      status: (post.metadata.published === false ? "DRAFT" : "LIVE") as "DRAFT" | "LIVE",
    }))
  );
          ...
          <div className="space-y-4">
            ...
            <div className="border border-primary/20 bg-primary/5 p-4 flex flex-col gap-3">
              ...
              <p className="text-xs text-muted-foreground">
                You have 3 unpublished drafts and 1 friend request pending.
              </p>
...
function ContentRow({ title, views, engagement, status }: ContentRowProps) {
  return (
    <tr className="group border-b border-border/20 last:border-0 hover:bg-primary/[0.02] transition-colors">
      ...
      <td className="py-4">
        <span
          className={cn(
            "text-[9px] px-2 py-0.5 border font-bold",
            status === "LIVE"
